...
namespace Xamarin.Juice {
	...
	public static class MayUtilities 
	{
		...
		/// </summary>
		public static May<T> MayAggregate<T>(this IEnumerable<T> sequence, 
			Func<T, T, T> folder) 
		{
		...
		/// <summary>
		/// Returns the minimum value in a sequence, as determined by 
		/// the given comparer or else the type's default comparer.
		/// If the sequence is empty, the result is NoValue.
		/// </summary>
		public static May<T> MayMin<T>(this IEnumerable<T> sequence, 
		...
		/// <summary>
		/// Returns the maximum value in a sequence, as determined by 
		/// the given comparer or else the type's default comparer.
		/// If the sequence is empty, the result is NoValue.
		/// </summary>
		public static May<T> MayMax<T>(this IEnumerable<T> sequence, 
			IComparer<T> comparer = null) {
			if (sequence == null) throw new ArgumentNullException("sequence");
			var c = comparer ?? Comparer<T>.Default;
			return sequence.MayAggregate((e1, e2) => c.Compare(e1, e2) >= 0 ? e1 : e2);
		...
		/// <summary>
		/// Returns the minimum value in a sequence, as determined by 
		/// projecting the items and using the given comparer or else 
		...
		/// If the sequence is empty, the result is NoValue.
		/// </summary>
		public static May<TItem> MayMaxBy<TItem, TCompare>(this IEnumerable<TItem> sequence, 
			Func<TItem, TCompare> projection, IComparer<TCompare> comparer = null) 
		{
			if (sequence == null) throw new ArgumentNullException("sequence");
			var c = comparer ?? Comparer<TCompare>.Default;
			return sequence
		...
		///<summary>Returns the first item in a sequence, or else NoValue 
		...
		///<summary>Returns the last item in a sequence, or else NoValue 
		///if the sequence is empty.</summary>
		...
		///<summary>Returns the single item in a sequence, NoValue 
		///if the sequence is empty, or throws an exception 
		///if there is more than one item.</summary>
		public static May<T> MaySingle<T>(this IEnumerable<T> sequence) 
		{
			...
			using (var e = sequence.GetEnumerator()) {
				...
				//note: this case is an exception to match the
				//semantics of SingleOrDefault, not because it's the best approach
				if (e.MoveNext()) throw new ArgumentOutOfRangeException("sequence", 
					"Expected either no items or a single item.");
				return result;
			}
		}
